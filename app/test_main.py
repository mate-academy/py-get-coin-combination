...
def test_check_type_of_function():
    assert isinstance(get_coin_combination(50), list)


def test_check_right_values_():
    assert len(get_coin_combination(1)) == 4


def test_check_right_values_type():
    assert isinstance(get_coin_combination(1123)[1], int)


def test_check_right_values_from_small_to_big():
...
def test_check_values_in_non_normal_range():
